...
<body>
<form name="allinfo" action="getAllInfo2.do"></form>

<div id="hd"><h1>Travel 게시판</h1></div>	
	<div align="right" id="hd">
		<form name="writepost" action="writepost.jsp" method="get">
			...
			<input type="hidden" name="gesipan" value="B">
		</form>
		<br>
...
<div id="header"><h1><a href="mainpage.jsp" style="color:black;text-decoration:none;">Travel 게시판</a></h1></div>
<div id="menubar">
<nav>
                <ul>
...
<c:if test="${requestScope.title != null }">
	...
	<form action="updatepost.jsp" name="uppost">
		...
		<br><br>
		&nbsp;&nbsp;작성자:${sbv.id }&nbsp;|&nbsp;${sbv.d }
		&nbsp;&nbsp;<a name="modifypost" onclick="upp();" style="cursor: pointer;" >수정</a>&nbsp;|<a name="deletepost" style="cursor: pointer;" onclick="location.href='deletepost.do?board_id=${sbv.board_id }&whatpost=B'">삭제</a>
		<hr class="my-hr2">
		<c:if test="${sbv.filename!=null }">
		<br>
		<video width="30%" height="30%" autoplay controls loop>
...
<table class="table table-hover" id="content" style="font-size:17px; text-align:center">
     ...
     <td style="font-size:18px;">날짜</td>
	     ...
	     <c:forEach var="vo1" items="${requestScope.alist1 }">
		     <tr>
			     <td>${vo1.board_id }</td>
			     <td><a href="titleinfo.do?title=${vo1.title }&page=${requestScope.page }&kindsofsearch=${requestScope.kindsofsearch }&search=${requestScope.search}&whatpost=B">${vo1.title }</a></td>
			     <td>${vo1.id }</td>
			     <td>${vo1.d}</td>
		     </tr>
	     </c:forEach>
...
<div class="align-center">
  <ul class="pagination">
 	<c:forEach var="i" begin="1" end="${requestScope.maxpage }" step="1">
  	<li>
  		<c:if test ="${requestScope.kindsofsearch == null }">
  			<a href="getAllInfo2.do?page=${i }&whatpost=">${i }</a>
  		</c:if>
  		<c:if test ="${requestScope.kindsofsearch != null }">
  			<a href="search.do?page=${i }&search=${requestScope.search }&kindsofsearch=${requestScope.kindsofsearch}&whatpost=">${i }</a>
  		</c:if>
  	</li>
	</c:forEach>
  </ul>
